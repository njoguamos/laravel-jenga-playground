<x-app-layout :showBackUrl="true">
    ...
    <form class="bg-white rounded-lg grid md:grid-cols-3 gap-6 p-6" action="{{ route('forex') }}" method="post">
        ...
        <div class="mb-2 md:col-span-2">
            ...
            <input type="text" id="accountNumber" name="accountNumber" value="{{ old('accountNumber') ?? '1450160649886' }} " class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
            @error('accountNumber')
            <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
            @enderror
        </div>
